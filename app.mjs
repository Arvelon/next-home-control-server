...
app.get("/delete", (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, message: "All records deleted from the database." });
  } catch (err) {
    console.error(err.message);
    res.status(500).json({ success: false, message: "Error deleting records from the database." });
  }
});
...
app.get("/allSensor1Entries", (req, res) => {
  try {
    ...
    console.error(err.message);
    res.status(500).json({ success: false, message: "Error reading from the database (sensor 1)." });
  }
});
...
app.get("/allSensor2Entries", (req, res) => {
  try {
    const rows = db.prepare("SELECT * FROM climate_sensor_2 ORDER BY timestamp DESC").all();
    const filtered = rows.filter(row => row.temperature > 0 && row.humidity < 150);
    res.status(200).json({ success: true, data: filtered });
  } catch (err) {
    console.error(err.message);
    res.status(500).json({ success: false, message: "Error reading from the database (sensor 2)." });
  }
});
...
app.get("/allSensor3Entries", (req, res) => {
  try {
    const rows = db.prepare("SELECT * FROM climate_sensor_3 ORDER BY timestamp DESC").all();
    const filtered = rows.filter(row => row.temperature > 0 && row.humidity < 150);
    res.status(200).json({ success: true, data: filtered });
  } catch (err) {
    console.error(err.message);
    res.status(500).json({ success: false, message: "Error reading from the database (sensor 3)." });
  }
});
...
app.get("/aggregated/all", (req, res) => {
  try {
    const rows = db.prepare("SELECT date(timestamp) AS date, AVG(temperature) AS avg_temperature, AVG(humidity) AS avg_humidity FROM climate_sensor_1 GROUP BY date").all();
...
app.get("/updateEjaculationCount", (req, res) => {
  ...
  try {
    ...
    console.error(err.message);
    res.status(500).json({ success: false, message: "Error updating ejaculation count." });
  }
});
...
app.get("/n/:stackSize", (req, res) => {
  ...
  if (isNaN(stackSize) || stackSize <= 0) {
    return res.status(400).json({ success: false, message: "Invalid :stackSize parameter." });
  }

  try {
    const sensor1Entries = db.prepare(`SELECT * FROM climate_sensor_1 ORDER BY timestamp DESC LIMIT ${stackSize}`).all();
    const sensor2Entries = db.prepare(`SELECT * FROM climate_sensor_2 ORDER BY timestamp DESC LIMIT ${stackSize}`).all();
    const sensor3Entries = db.prepare(`SELECT * FROM climate_sensor_3 ORDER BY timestamp DESC LIMIT ${stackSize}`).all();
